...
class RawFilesClient:
    ...
    def get_task_user_uploaded_file_presigned_url(
    ...
    ) -> HttpResponse[TaskUploadedFileResponse]:
        """
        ...
        """
        _response = self._client_wrapper.httpx_client.request(
            f"files/task-uploaded-files/{jsonable_encoder(task_id)}/{jsonable_encoder(file_id)}",
            method="GET",
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                _data = typing.cast(
                    ...
                    construct_type(
                        type_=TaskUploadedFileResponse,  # type: ignore
                        object_=_response.json(),
                    ),
                )
                return HttpResponse(response=_response, data=_data)
    ...
    def get_task_output_file_presigned_url(
        ...
        """
        _response = self._client_wrapper.httpx_client.request(
            f"files/task-output-files/{jsonable_encoder(task_id)}/{jsonable_encoder(file_id)}",
            method="GET",
            request_options=request_options,
        )
        try:
...
class AsyncRawFilesClient:
    ...
    async def get_task_user_uploaded_file_presigned_url(
        self, task_id: str, file_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> AsyncHttpResponse[TaskUploadedFileResponse]:
        """
        Get secure download URL for a user uploaded file used in the task.
